   ...
   public void addNodeByPosition(T a, int index) {        
    ...
    public T getNodeAtIndex(int index){
        if(head==null || index>=length() || index<0){
            ...
            int i = 0;
            ListNode currentNode = head;
            while(currentNode.getLink() != null && i != index){
                currentNode = currentNode.getLink();
                i++;
            }
            return (T) currentNode.getData();
    ...
    public void bubbleSort(int a){
        ...
        for(int iteration=0; iteration<length(); iteration++){
            for(int i=0; i<length()-1-iteration; i++){
                if(getNodeAtIndex(i).compareTo(getNodeAtIndex(i+1))>0 && orderType == ASCENDING){
                    swap(i, i+1);
                }else if(getNodeAtIndex(i).compareTo(getNodeAtIndex(i+1))<0 && orderType == DESCENDING){
                    swap(i, i+1);
                }
            }
        }
