...
public class BST<T extends Comparable<T>> {
    ...
    public T getMinimum(){
        return (T) getMinimumNumber(root, (T) root.getData());
    }
    public T getMinimumNumber(BSTNode<T> a, T data){
        if(a.getLeft() == null){
            return (T) a.getData();
        }else{
            return (T) getMinimumNumber(a.getLeft(), (T) a.getLeft().getData());
        }
    }
     public T getMaximum(){
        return (T) getMaximumNumber(root, (T) root.getData());
    }
    public T getMaximumNumber(BSTNode<T> a, T data){
        if(a.getRight() == null){
            return (T) a.getData();
        }else{
            return (T) getMaximumNumber(a.getRight(), (T) a.getRight().getData());
        }
    }
    public int getTotal(){
        return (Integer) getTotal(root, 0);
    }
    public int getTotal(BSTNode<T> a, int total){
        total += (Integer) a.getData();
        if(a.getLeft() != null){
            total = getTotal(a.getLeft(), total);
        }
        if(a.getRight() != null){
            total = getTotal(a.getRight(), total);
        }
        return total;
    }
    public int getOccurence(T input){
        return getOccurrence(root, input, 0);
    }
    public int getOccurrence(BSTNode<T> a, T input, Integer count){
        if(input.compareTo(a.getData()) == 0){
            ++count;
        }
        if(a.getLeft() != null){
            count = getOccurrence(a.getLeft(), input, count);
        }
        if(a.getRight() != null){
            count = getOccurrence(a.getRight(), input, count);
        }
        return count;
    }
